...
public class GetAllOrganizationResourcesEndpoint(ICommandDispatcher dispatcher) : EndpointBase
{
    [HttpGet("organization/{organizationId}/resources")]
    [SwaggerOperation(Tags = new[] { "Organization - Resources" })]
    public async Task<IActionResult> GetAllOrganizationResources([FromRoute] string organizationId)
    {
        // * Create the request
        var cmd = GetAllResourcesCommand.Create(organizationId, ResourceLevel.Organization);
