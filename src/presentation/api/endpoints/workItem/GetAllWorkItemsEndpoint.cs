...
public class GetAllWorkItemsEndpoint(ICommandDispatcher dispatcher) : EndpointBase
{
    ...
    public async Task<IActionResult> GetAllWorkItems([FromQuery] string? projectId)
    {
        ...
        {
            var cmd = GetProjectWorkItemsCommand.Create(projectId);

            // * Dispatch the command
            var result = await dispatcher.DispatchAsync<GetProjectWorkItemsCommand>(cmd.Value);
    ...
    private List<WorkItemDTO> TransformList(GetProjectWorkItemsCommand cmd)
    {
        // * Extract the workItems from the command
        var workItems = cmd.WorkItems;
