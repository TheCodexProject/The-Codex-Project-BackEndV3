...
public class DeleteResourceHandler(IUnitOfWork unitOfWork) : ICommandHandler<DeleteResourceCommand>
{
    public async Task<Result> HandleAsync(DeleteResourceCommand command)
    {
        ...
        // * Find the resource in the owner's list of resources
        ...
        // * Delete the resource from the owner
        var deleteResourceResult = owner.Value.RemoveResource(resource);

        // ? Were there any validation errors?
        if(deleteResourceResult.IsFailure)
            // ! Return the error
            return Result.Failure(deleteResourceResult.Errors.ToArray());
